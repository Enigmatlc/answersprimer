...
public:
	...
	size_t capacity()const { return cap - elements; }//capacity of StrVec
	...
	std::string& at(size_t p) { return *(elements + p); }
	const std::string& at(size_t p) const { return *(elements + p); }
private:
...
void StrVec::free() {
	...
	std::for_each(elements, first_free, [this](std::string& c) {alloc.destroy(&c);});
